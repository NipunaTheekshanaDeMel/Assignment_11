...
customer_btn.eq(0).on('click', () => {
    ...
    if (getCustomerIndex(customerId) < 0){
        if (confirm('Do you want to add a customer?')){
        ...
        alert('Ooops!! Customer is Already exists 😓');
    }
});
...
customer_btn.eq(1).on('click', () => {
    ...
    if (index >= 0){
        if (confirm(`Do you want to update ${customerId} ?`)){
            customer_db[index] = customer;
            loadCustomerTable();
            customer_btn.eq(3).click();
        }
    }else{
        alert('Ooops!! Customer did not exists 😓');
    }
});
...
customer_btn.eq(2).on('click', () => {
    ...
    if (index >= 0){
        if (confirm(`Do you want to delete ${customerId} ?`)){
